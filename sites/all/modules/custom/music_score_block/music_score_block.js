        ...
        $('.music-score-block-form').submit(function() {
            ...
            if ($(this).find("input:first").val() == "" || $(this).find("input:first").val() == Drupal.settings.music_score_block.music_score_search_hint) {
                ...
                if ($('#books-plus-error').length == 0) {
                    $('.four_five_three_stacked-region--top').prepend("<div id='books-plus-error' class='alert--message'><h3 class='alert-title'><i class='icon-warning'></i> " + Drupal.settings.music_score_block.music_score_search_error + "</h3></div>");
                }
                return false;
            }
