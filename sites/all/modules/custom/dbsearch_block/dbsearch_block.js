     ...
     $('#dbsearch-block-form').submit(function() {
    	...
    	if ($(this).find("input:first").val() == "" || $(this).find("input:first").val() == Drupal.settings.dbsearch_block.db_search_hint) {
				if($('#dbsearch-error').length == 0) {
		 			$('.four_five_three_stacked-region--top').prepend("<div id='dbsearch-error' class='alert--message'><h3 class='alert-title'><i class='icon-warning-sign'></i> "+Drupal.settings.dbsearch_block.db_search_error+"</h3></div>");
    		}
				return false;
    	}
