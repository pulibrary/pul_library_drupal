     ...
     $('#allsearch-block-form').submit(function() {
        if ($(this).find("input:first").val() == "" || $(this).find("input:first").val() == Drupal.settings.allsearch_block.all_search_hint) {
                if($('#allsearch-error').length == 0) {
                  $("<div id='allsearch-error' class='alert--message'><h2 class='element-invisible'>Error message</h2><h3><i class='icon-alert'></i> "+Drupal.settings.allsearch_block.all_search_error+"</h3></div>").insertBefore('.l-content');
            }
